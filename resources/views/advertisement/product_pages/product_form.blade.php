...
@push('custom-scripts')
    ...
    <script type="text/javascript">
            ...
            $('#formadd').validate({// initialize the plugin
                ...
                messages: {
                    ...
                    phone: {
                        ...
                        remote: "{{ __('register.validation.phone_exists_login') }}"
                    },
                    email: {
                        required: "{{ __('register.validation.email_required') }}",
                        remote: "{{ __('register.validation.email_exists_login') }}"
                    },

                    business_types_id: "{{ __('product.validation.business_type_required') }}",
                    state_id: "{{ __('common.validation.state_required') }}",
                    district_id: "{{ __('common.validation.district_required') }}",
                    ...
                    uploaded_files: "{{ __('product.validation.product_image_required') }}",
                },
        ...
        document.querySelectorAll('input[name="lead_type"]').forEach(function (elem) {
            elem.addEventListener('change', function () {
                document.querySelectorAll('.nav-link').forEach(function (label) {
                    label.classList.remove('active', 'show');
                });

                const selectedLabel = document.querySelector('label[for="' + this.id + '"]');
                selectedLabel.classList.add('active', 'show');
            });
        });
...
@section('content')
    ...
    <div class="content d-flex justify-content-center align-items-center">
        <!-- Product form -->
        <form method="post" class="flex-fill dropzone" action="{{route('ads.saveProduct')}}" id="formadd" enctype="multipart/form-data">
            {{csrf_field()}}
            <div class="row">
                    ...
                    <div class="card mb-0">
                        <div class="card-body">
                            ...
                            <div class="row">
                                <div class="col-md-6">
                                    ...
                                    <div class="nav nav-pills nav-pills-bordered mt-2">
                                        <input type="radio" class="d-none" name="lead_type" value="0" id="active_tab" checked>
                                        <label for="active_tab" class="nav-link rounded-round active show mr-2" style="cursor: pointer;"><b>{{ __('common.sell') }}</b></label>

                                        <input type="radio" class="d-none" name="lead_type" value="1" id="inactive_tab">
                                        <label for="inactive_tab" class="nav-link rounded-round" style="cursor: pointer;"><b>{{ __('common.buy') }}</b></label>
                                    </div>
                                </div>

                                <div class="col-md-6">
                                    <b>{{ __('common.select_role_heading') }}</b>
                                    <div class="nav nav-pills nav-pills-bordered mt-2">
                                        <select class="form-control required select2 select" name="business_types_id" id="business_types">
                            ...
                            <div class="row">
                                <div class="col-md-12">
                                    <div class="form-group form-group-feedback form-group-feedback-right">
                                        <input type="text" name="title" class="form-control" placeholder="{{ __('product.ad_title_placeholder') }}">
                            ...
                            <div class="row">
                                ...
                                <div class="form-group col-lg-6">
                                    <select class="form-control required select2 select" name="sub_category_id" id="sub_category">
                                        <option value="">{{ __('common.select_subcategory') }}</option>
                                    </select>
                                </div>
                            </div>
                            ...
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="form-group form-group-feedback form-group-feedback-right">
                                        <input type="text" name="variety" class="form-control" placeholder="{{ __('product.enter_variety') }}">
                                        <div class="form-control-feedback">
                                            <i class="icon-pen-plus text-muted"></i>
                                        </div>
                                    </div>
                                ...
                                <div class="col-md-6">
                                    <div class="form-group form-group-feedback form-group-feedback-right">
                                        <input type="text" name="quantity" class="form-control" placeholder="{{ __('product.enter_quantity') }}">
                                        <div class="form-control-feedback">
                                            <i class="icon-pen-plus text-muted"></i>
                                        </div>
                                    </div>
                                ...
                                <div class="form-group col-lg-6">
                                    <select class="form-control required select2 select" name="unit_id" id="unit-price">
                                        ...
                                        @foreach($units as $unit)
                                            <option value="{{$unit->id}}" data-en="{{ $unit->en_name }}" data-hi="{{ $unit->hi_name }}" data-mr="{{ $unit->mr_name }}">{{ $unit[$locale . '_name'] }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="col-md-6">
                                    <div class="form-group form-group-feedback form-group-feedback-right">
                                        <input type="text" name="price" id="price-input" class="form-control" placeholder="{{ __('product.enter_price') }}">
                                        <div class="form-control-feedback">
                                            <i class="icon-pen-plus text-muted"></i>
                                        </div>
                                    </div>
                            ...
                            <div class="row">
                                <div class="form-group col-lg-12 mb-3">
                                    ...
                                    <div class="custom-control custom-radio custom-control-inline">
                                        <input type="radio" class="custom-control-input" name="is_organic" id="organic_yes" value="1" checked="">
                                        <label class="custom-control-label" for="organic_yes">{{ __('product.yes') }}</label>
                                    </div>

                                    <div class="custom-control custom-radio custom-control-inline">
                                        <input type="radio" class="custom-control-input" name="is_organic" id="organic_no" value="0">
                            ...
                            <div class="row">
                                <div class="form-group col-lg-12 mb-3">
                                    ...
                                    <div class="custom-control custom-radio custom-control-inline">
                                        <input type="radio" class="custom-control-input" name="selling_frequency" id="sell_once" value="Once" checked="">
                                        <label class="custom-control-label" for="sell_once">{{ __('product.once') }}</label>
                                    </div>

                                    <div class="custom-control custom-radio custom-control-inline">
                                        <input type="radio" class="custom-control-input" name="selling_frequency" id="sell_daily" value="Daily">
                                        <label class="custom-control-label" for="sell_daily">{{ __('product.daily') }}</label>
                                    </div>
                                    <div class="custom-control custom-radio custom-control-inline">
                                        <input type="radio" class="custom-control-input" name="selling_frequency" id="sell_weekly" value="Weekly">
                                        <label class="custom-control-label" for="sell_weekly">{{ __('product.weekly') }}</label>
                                    </div>

                                    <div class="custom-control custom-radio custom-control-inline">
                                        <input type="radio" class="custom-control-input" name="selling_frequency" id="sell_monthly" value="Monthly">
                                        <label class="custom-control-label" for="sell_monthly">{{ __('product.monthly') }}</label>
                                    </div>

                                    <div class="custom-control custom-radio custom-control-inline">
                                        <input type="radio" class="custom-control-input" name="selling_frequency" id="sell_seasonal" value="Seasonal">
                                        <label class="custom-control-label" for="sell_seasonal">{{ __('product.seasonal') }}</label>
                                    </div>

                                    <div class="custom-control custom-radio custom-control-inline">
                                        <input type="radio" class="custom-control-input" name="selling_frequency" id="sell_ondemand" value="On-demand">
                            ...
                            <div class="text-right">
								<button type="submit" class="btn bg-success-400">{{ __('product.submit') }}
