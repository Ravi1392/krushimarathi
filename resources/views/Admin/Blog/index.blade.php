...
$(function () {
    ...
    function delete_recodes() {
        ...
        if (Array.prototype.forEach) {
         ...
         $(".switch").change(function () {
            ...
            var url = "{{URL::to('admin/blogActiveDeactive')}}";
            url = url + "/" + id + "/" + status;
            $.ajax({
                url: url
            }).done(function (data) {
                if (data == 1)
                {
                    if (status == 1)
                    {
                        toastr.success('Blog has been Activated', 'Activated');
                    } else
                    {
                        toastr.error('Blog has been Deactivated', 'Deactivated');
                    }
                } else
                {
        ...
        $('.reset_views').on('click', function () {
            ...
            function (isConfirm) {
                if (isConfirm) {
                    var url = "{{URL::to('admin/blogResetViews')}}";
                    $.ajax({
                        url: url,
                        type: "get",
                        data: {
    ...
    $(document).on("click", ".copy-btn", function() {
        ...
        navigator.clipboard.writeText(text).then(function() {
            ...
            console.error('कॉपी करता आली नाही:', error);
        });
    });
    ...
    <div class="panel panel-flat">
        <div class="panel-heading">
            ...
            <div class="heading-elements">
                ...
                @endif
                <a href="{{route('admin.blog.addBlog')}}" class="btn btn-labeled btn-labeled-right bg-blue heading-btn">Add Blog<b><i class="icon-menu7"></i></b></a>
            </div>
        </div>
