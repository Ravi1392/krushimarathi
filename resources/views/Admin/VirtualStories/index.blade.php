...
$(function () {
    ...
    $('.pending-table').DataTable({
        ...
        "select": true,
        "ajax": {
            "url": "{{route('admin.virtualStories.getData')}}",
            "dataType": "json",
            "type": "POST",
    ...
    function delete_recodes() {
            ...
            function (isConfirm) {
                if (isConfirm) {
                    $.ajax({
                        ...
                        swal({
                            title: "Deleted!",
                            text: "Story has been successfully deleted..",
                            type: "success",
                            showCancelButton: true,
                            closeOnConfirm: false,
        ...
        if (Array.prototype.forEach) {
         ...
         $(".switch").change(function () {
            ...
            var url = "{{URL::to('admin/storyActiveDeactive')}}";
            url = url + "/" + id + "/" + status;
            $.ajax({
                url: url
            }).done(function (data) {
                if (data == 1)
                {
                    if (status == 1)
                    {
                        toastr.success('Story has been Activated', 'Activated');
                    } else
                    {
                        toastr.error('Story has been Deactivated', 'Deactivated');
                    }
                } else
                {
    ...
    $('.reset_views').on('click', function () {
        ...
        function (isConfirm) {
            if (isConfirm) {
                var url = "{{URL::to('admin/storyResetViews')}}";
                $.ajax({
                    url: url,
                    type: "get",
                    data: {
...
<div class="content">
    ...
    <div class="panel panel-flat">
        <div class="panel-heading">
            <h5 class="panel-title">Web Story List</h5>
            <div class="heading-elements">
                ...
                <a href="{{route('admin.virtualStories.addVirtualStories')}}" class="btn btn-labeled btn-labeled-right bg-blue heading-btn">Add Web Story<b><i class="icon-menu7"></i></b></a>
            </div>
        </div>
