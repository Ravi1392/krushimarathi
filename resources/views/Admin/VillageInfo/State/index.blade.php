...
$(function () {
    ...
    function delete_recodes() {
            ...
            function (isConfirm) {
                if (isConfirm) {
                    $.ajax({
                        ...
                        swal({
                            title: "Deleted!",
                            text: "State data has been successfully deleted..",
                            type: "success",
                            showCancelButton: true,
                            closeOnConfirm: false,
        ...
        if (Array.prototype.forEach) {
         ...
         $(".switch").change(function () {
            ...
            var url = "{{URL::to('admin/stateactivedeactive')}}";
            url = url + "/" + id + "/" + status;
            $.ajax({
                url: url
            }).done(function (data) {
                if (data == 1)
                {
                    if (status == 1)
                    {
                        toastr.success('State data has been Activated', 'Activated');
                    } else
                    {
                        toastr.error('State data has been Deactivated', 'Deactivated');
                    }
                } else
                {
    ...
    $('.reset_views').on('click', function () {
        ...
        function (isConfirm) {
            if (isConfirm) {
                var url = "{{URL::to('admin/stateResetViews')}}";
                $.ajax({
                    url: url,
                    type: "get",
                    data: {
    ...
    <div class="panel panel-flat">
        <div class="panel-heading">
            ...
            <div class="heading-elements">
                ...
                <a href="{{route('admin.state.addState')}}" class="btn btn-labeled btn-labeled-right bg-blue heading-btn">Add State<b><i class="icon-menu7"></i></b></a>
            </div>
        </div>
