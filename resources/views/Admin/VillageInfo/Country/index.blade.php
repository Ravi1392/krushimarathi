...
$(function () {
    ...
    function delete_recodes() {
        ...
        $('.delete_row').on('click', function () {
            ...
            swal({
                title: "Are you sure?",
                text: "You will not be able to recover this country data!",
                type: "warning",
            ...
            function (isConfirm) {
                if (isConfirm) {
                    $.ajax({
                        ...
                        swal({
                            title: "Deleted!",
                            text: "Country data has been successfully deleted..",
                            type: "success",
                            showCancelButton: true,
                            closeOnConfirm: false,
                    ...
                    swal({
                        title: "Cancelled",
                        text: "Your country data is safe :)",
                        confirmButtonColor: "#2196F3",
                        type: "error"
                    });
                }
        ...
        if (Array.prototype.forEach) {
         ...
         $(".switch").change(function () {
            ...
            var url = "{{URL::to('admin/countryactivedeactive')}}";
            url = url + "/" + id + "/" + status;
            $.ajax({
                url: url
            }).done(function (data) {
                if (data == 1)
                {
                    if (status == 1)
                    {
                        toastr.success('Country data has been Activated', 'Activated');
                    } else
                    {
                        toastr.error('Country data has been Deactivated', 'Deactivated');
                    }
                } else
                {
    ...
    $('.reset_views').on('click', function () {
        ...
        function (isConfirm) {
            if (isConfirm) {
                var url = "{{URL::to('admin/countryResetViews')}}";
                $.ajax({
                    url: url,
                    type: "get",
                    data: {
    ...
    <div class="panel panel-flat">
        <div class="panel-heading">
            ...
            <div class="heading-elements">
                ...
                <a href="{{route('admin.country.addCountry')}}" class="btn btn-labeled btn-labeled-right bg-blue heading-btn">Add Country<b><i class="icon-menu7"></i></b></a>
            </div>
        </div>
