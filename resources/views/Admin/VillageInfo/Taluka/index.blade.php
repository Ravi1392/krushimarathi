...
$(function () {
    ...
    $('.pending-table').DataTable({
        ...
        "select": true,
        "ajax": {
            "url": "{{route('admin.taluka.getData')}}",
            "dataType": "json",
            "type": "POST",
    ...
    function delete_recodes() {
            ...
            function (isConfirm) {
                if (isConfirm) {
                    $.ajax({
                        ...
                        swal({
                            title: "Deleted!",
                            text: "Taluka data has been successfully deleted..",
                            type: "success",
                            showCancelButton: true,
                            closeOnConfirm: false,
        ...
        $(".switch").change(function () {
            ...
            var url = "{{URL::to('admin/talukaactivedeactive')}}";
            url = url + "/" + id + "/" + status;
            $.ajax({
                url: url
            }).done(function (data) {
                if (data == 1)
                {
                    if (status == 1)
                    {
                        toastr.success('Taluka data has been Activated', 'Activated');
                    } else
                    {
                        toastr.error('Taluka data has been Deactivated', 'Deactivated');
                    }
                    // Reload DataTable or update UI
                    $('.pending-table').DataTable().ajax.reload();
                } else
    ...
    $('.reset_views').on('click', function () {
        ...
        function (isConfirm) {
            if (isConfirm) {
                var url = "{{URL::to('admin/talukaResetViews')}}";
                $.ajax({
                    url: url,
                    type: "get",
                    data: {
...
<div class="content">
    ...
    <div class="panel panel-flat">
        <div class="panel-heading">
            <h5 class="panel-title">Taluka list</h5>
            <div class="heading-elements">
                ...
                <a href="{{route('admin.taluka.addTaluka')}}" class="btn btn-labeled btn-labeled-right bg-blue heading-btn">Add Taluka<b><i class="icon-menu7"></i></b></a>
            </div>
        </div>
