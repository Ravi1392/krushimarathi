...
$(function () {
    ...
    function delete_recodes() {
        ...
        if (Array.prototype.forEach) {
         ...
         $(".switch").change(function () {
            ...
            var url = "{{URL::to('admin/categoryactivedeactive')}}";
            url = url + "/" + id + "/" + status;
            $.ajax({
                url: url
            }).done(function (data) {
                if (data == 1)
                {
                    if (status == 1)
                    {
                        toastr.success('Category has been Activated', 'Activated');
                    } else
                    {
                        toastr.error('Category has been Deactivated', 'Deactivated');
                    }
                } else
                {
    ...
    $('.reset_views').on('click', function () {
            ...
            function (isConfirm) {
                if (isConfirm) {
                    var url = "{{URL::to('admin/categoryResetViews')}}";
                    $.ajax({
                        url: url,
                        type: "get",
                        data: {
    ...
    <div class="panel panel-flat">
        <div class="panel-heading">
            ...
            <div class="heading-elements">
                ...
                <a href="{{route('admin.category.addcategory')}}" class="btn btn-labeled btn-labeled-right bg-blue heading-btn ajaxviewmodel">Add Category<b><i class="icon-menu7"></i></b></a>
            </div>
        </div>
