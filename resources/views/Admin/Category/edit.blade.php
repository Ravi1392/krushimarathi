    ...
    $(document).ready(function () {
        $('#formadd').validate({// initialize the plugin
            rules: {
                name: {
                    required: true,
                    remote: '{{route("admin.category.categoryCheckUpdate",$update->id)}}'
                },
                category_slug: {
                    required: true,
                    minlength: 4,
                    maxlength: 28,
                    pattern: /^[a-zA-Z0-9\- ]+$/,
                    remote: '{{route("admin.category.categorySlugCheckUpdate",$update->id)}}',
                },
                meta_keywords: {
                    required: true,
                },
...
<div class="modal-content">
    ...
    <div class="modal-body">
        <form method="post"  action="{{route('admin.category.editsave',base64_encode($update->id))}}" id="formadd" enctype="multipart/form-data">
            {{csrf_field()}}
