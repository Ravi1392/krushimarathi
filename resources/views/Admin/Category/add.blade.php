    ...
    $(document).ready(function () {
        $('#formadd').validate({// initialize the plugin
            rules: {
                name: {
                    required: true,
                    remote: '{{route("admin.category.categorycheck")}}',
                },
                category_slug: {
                    required: true,
                    minlength: 4,
                    maxlength: 28,
                    pattern: /^[a-zA-Z0-9\- ]+$/,
                    remote: '{{route("admin.category.categoryslugcheck")}}',
                },
                meta_keywords: {
                    required: true,
                },
...
<div class="modal-content">
    ...
    <div class="modal-body">
        <form method="post"  action="{{route('admin.category.save')}}" id="formadd" enctype="multipart/form-data">
            {{csrf_field()}}
