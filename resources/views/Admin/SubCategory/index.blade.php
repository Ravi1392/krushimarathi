...
$(function () {
    ...
    function delete_recodes() {
            ...
            function (isConfirm) {
                if (isConfirm) {
                    $.ajax({
                        ...
                        swal({
                            title: "Deleted!",
                            text: "Sub Category has been successfully deleted..",
                            type: "success",
                            showCancelButton: true,
                            closeOnConfirm: false,
        ...
        if (Array.prototype.forEach) {
         ...
         $(".switch").change(function () {
            ...
            var url = "{{URL::to('admin/subCategoryActiveDeactive')}}";
            url = url + "/" + id + "/" + status;
            $.ajax({
                url: url
            }).done(function (data) {
                if (data == 1)
                {
                    if (status == 1)
                    {
                        toastr.success('Sub Category has been Activated', 'Activated');
                    } else
                    {
                        toastr.error('Sub Category has been Deactivated', 'Deactivated');
                    }
                } else
                {
    ...
    $('.reset_views').on('click', function () {
        ...
        function (isConfirm) {
            if (isConfirm) {
                var url = "{{URL::to('admin/subCategoryResetViews')}}";
                $.ajax({
                    url: url,
                    type: "get",
                    data: {
...
<div class="content">
    ...
    <div class="panel panel-flat">
        <div class="panel-heading">
            <h5 class="panel-title">Sub Category list</h5>
            <div class="heading-elements">
                ...
                <a href="{{route('admin.subcategory.addsubcategory')}}" class="btn btn-labeled btn-labeled-right bg-blue heading-btn">Add Sub Category<b><i class="icon-menu7"></i></b></a>
            </div>
        </div>
