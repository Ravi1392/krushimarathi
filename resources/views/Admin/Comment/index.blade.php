...
$(function () {
    ...
    $('.pending-table').DataTable({
        ...
        "select": true,
        "ajax": {
            "url": "{{route('admin.comments.getData')}}",
            "dataType": "json",
            "type": "POST",
    ...
    function delete_recodes() {
        ...
        if (Array.prototype.forEach) {
         ...
         $(".switch").change(function () {
            ...
            var url = "{{URL::to('admin/commentActiveDeactive')}}";
            url = url + "/" + id + "/" + status;
            $.ajax({
                url: url
            }).done(function (data) {
                if (data == 1)
                {
                    if (status == 1)
                    {
                        toastr.success('Comment has been Activated', 'Activated');
                    } else
                    {
                        toastr.error('Comment has been Deactivated', 'Deactivated');
                    }
                } else
                {
...
<div class="content">
    ...
    <div class="panel panel-flat">
        <div class="panel-heading">
            <h5 class="panel-title">Comment list</h5>
            <div class="heading-elements">
                <a href="{{route('admin.comments.addComment')}}" class="btn btn-labeled btn-labeled-right bg-blue heading-btn ajaxviewmodel">Add Comment<b><i class="icon-menu7"></i></b></a>
            </div>
        </div>
