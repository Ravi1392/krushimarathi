...
$(function () {
    ...
    $('.pending-table').DataTable({
        ...
        "select": true,
        "ajax": {
            "url": "{{route('admin.special_categories.getData')}}",
            "dataType": "json",
            "type": "POST",
    ...
    function delete_recodes() {
            ...
            function (isConfirm) {
                if (isConfirm) {
                    $.ajax({
                        ...
                        swal({
                            title: "Deleted!",
                            text: "Special Category has been successfully deleted.",
                            type: "success",
                            showCancelButton: true,
                            closeOnConfirm: false,
        ...
        if (Array.prototype.forEach) {
         ...
         $(".switch").change(function () {
            ...
            var url = "{{URL::to('admin/specialCategoryActiveSeactive')}}";
            url = url + "/" + id + "/" + status;
            $.ajax({
                url: url
            }).done(function (data) {
                if (data == 1)
                {
                    if (status == 1)
                    {
                        toastr.success('Special Category has been Activated', 'Activated');
                    } else
                    {
                        toastr.error('Special Category has been Deactivated', 'Deactivated');
                    }
                } else
                {
    ...
    $('.reset_views').on('click', function () {
        ...
        function (isConfirm) {
            if (isConfirm) {
                var url = "{{URL::to('admin/specialCategoryResetViews')}}";
                $.ajax({
                    url: url,
                    type: "get",
                    data: {
...
<div class="content">
    ...
    <div class="panel panel-flat">
        <div class="panel-heading">
            <h5 class="panel-title">Special Category list</h5>
            <div class="heading-elements">
                ...
                <a href="{{route('admin.special_categories.addSpecialCategory')}}" class="btn btn-labeled btn-labeled-right bg-blue heading-btn ajaxviewmodel">Add Special Category<b><i class="icon-menu7"></i></b></a>
            </div>
        </div>
