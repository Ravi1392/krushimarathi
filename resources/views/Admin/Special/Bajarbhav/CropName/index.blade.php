...
$(function () {
    ...
    $('.pending-table').DataTable({
        ...
        "select": true,
        "ajax": {
            "url": "{{route('admin.crop_name.getData')}}",
            "dataType": "json",
            "type": "POST",
    ...
    function delete_recodes() {
        $('.delete_row').on('click', function () {
            ...
            swal({
                title: "Are you sure?",
                text: "You will not be able to recover this Crop Name!",
                type: "warning",
            ...
            function (isConfirm) {
                if (isConfirm) {
                    $.ajax({
                        ...
                        swal({
                            title: "Deleted!",
                            text: "Crop Name has been successfully deleted..",
                            type: "success",
                            showCancelButton: true,
                            closeOnConfirm: false,
        ...
        if (Array.prototype.forEach) {
         ...
         $(".switch").change(function () {
            ...
            var url = "{{URL::to('admin/cropNameActiveDeactive')}}";
            url = url + "/" + id + "/" + status;
            $.ajax({
                url: url
            }).done(function (data) {
                if (data == 1)
                {
                    if (status == 1)
                    {
                        toastr.success('Crop Name has been Activated', 'Activated');
                    } else
                    {
                        toastr.error('Crop Name has been Deactivated', 'Deactivated');
                    }
                } else
                {
...
<div class="content">
    ...
    <div class="panel panel-flat">
        <div class="panel-heading">
            <h5 class="panel-title">पिकांची नावे</h5>
            <div class="heading-elements">
                <a href="{{route('admin.crop_name.addCropName')}}" class="btn btn-labeled btn-labeled-right bg-blue heading-btn ajaxviewmodel">Add New Crop<b><i class="icon-menu7"></i></b></a>
            </div>
        </div>
                ...
                <table class="table datatable-basic pending-table table-framed" >
                    <thead>
                        <tr>
                            ...
                            <th>Marathi Crop Name</th>
                            <th>English Crop Name</th>
