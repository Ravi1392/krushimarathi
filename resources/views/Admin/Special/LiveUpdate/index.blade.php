...
$(function () {
    ...
    $('.pending-table').DataTable({
        ...
        "select": true,
        "ajax": {
            "url": "{{route('admin.live_update.getData')}}",
            "dataType": "json",
            "type": "POST",
    ...
    function delete_recodes() {
            ...
            function (isConfirm) {
                if (isConfirm) {
                    $.ajax({
                        ...
                        swal({
                            title: "Deleted!",
                            text: "Live Update has been successfully deleted.",
                            type: "success",
                            showCancelButton: true,
                            closeOnConfirm: false,
        ...
        $(".switch").change(function () {
            ...
            var url = "{{URL::to('admin/liveUpdateActiveDeactive')}}";
            url = url + "/" + id + "/" + status;
            $.ajax({
                url: url
            }).done(function (data) {
                if (data == 1)
                {
                    if (status == 1)
                    {
                        toastr.success('Live Update has been Activated', 'Activated');
                    } else
                    {
                        toastr.error('Live Update has been Deactivated', 'Deactivated');
                    }
                } else
                {
        ...
        $('.reset_views').on('click', function () {
            ...
            function (isConfirm) {
                if (isConfirm) {
                    var url = "{{URL::to('admin/updatesResetViews')}}";
                    $.ajax({
                        url: url,
                        type: "get",
                        data: {
...
<div class="content">
    ...
    <div class="panel panel-flat">
        <div class="panel-heading">
            <h5 class="panel-title">Live Update List</h5>
            <div class="heading-elements">
                ...
                <a href="{{route('admin.live_update.addLiveUpdate')}}" class="btn btn-labeled btn-labeled-right bg-blue heading-btn ajaxviewmodel">Add Live Update Data<b><i class="icon-menu7"></i></b></a>
            </div>
        </div>
