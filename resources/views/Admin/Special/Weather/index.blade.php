...
$(function () {
    ...
    $('.pending-table').DataTable({
        ...
        "select": true,
        "ajax": {
            "url": "{{route('admin.weather.getData')}}",
            "dataType": "json",
            "type": "POST",
    ...
    function delete_recodes() {
            ...
            function (isConfirm) {
                if (isConfirm) {
                    $.ajax({
                        ...
                        swal({
                            title: "Deleted!",
                            text: "Weather has been successfully deleted..",
                            type: "success",
                            showCancelButton: true,
                            closeOnConfirm: false,
        ...
        if (Array.prototype.forEach) {
         ...
         $(".switch").change(function () {
            ...
            var url = "{{URL::to('admin/weatherActiveDeactive')}}";
            url = url + "/" + id + "/" + status;
            $.ajax({
                url: url
            }).done(function (data) {
                if (data == 1)
                {
                    if (status == 1)
                    {
                        toastr.success('City Weather has been Activated', 'Activated');
                    } else
                    {
                        toastr.error('City Weather has been Deactivated', 'Deactivated');
                    }
                } else
                {
...
<div class="content">
    ...
    <div class="panel panel-flat">
        <div class="panel-heading">
            <h5 class="panel-title">Weather list</h5>
            <div class="heading-elements">
                <a href="{{route('cron.weather_update')}}" class="btn btn-labeled btn-labeled-right bg-blue heading-btn" target="_blank">Update Weather<b><i class="icon-menu7"></i></b></a>
            </div>
        </div>
