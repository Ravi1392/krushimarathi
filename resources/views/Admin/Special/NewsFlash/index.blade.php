...
$(function () {
    ...
    $('.pending-table').DataTable({
        ...
        "select": true,
        "ajax": {
            "url": "{{route('admin.news_flash.getData')}}",
            "dataType": "json",
            "type": "POST",
    ...
    function delete_recodes() {
            ...
            function (isConfirm) {
                if (isConfirm) {
                    $.ajax({
                        ...
                        swal({
                            title: "Deleted!",
                            text: "News Flash has been successfully deleted.",
                            type: "success",
                            showCancelButton: true,
                            closeOnConfirm: false,
        ...
        $(".switch").change(function () {
            ...
            var url = "{{URL::to('admin/newsFlashActiveDeactive')}}";
            url = url + "/" + id + "/" + status;
            $.ajax({
                url: url
            }).done(function (data) {
                if (data == 1)
                {
                    if (status == 1)
                    {
                        toastr.success('News Flash has been Activated', 'Activated');
                    } else
                    {
                        toastr.error('News Flash has been Deactivated', 'Deactivated');
                    }
                } else
                {
        ...
        $('.reset_views').on('click', function () {
            ...
            function (isConfirm) {
                if (isConfirm) {
                    var url = "{{URL::to('admin/flashResetViews')}}";
                    $.ajax({
                        url: url,
                        type: "get",
                        data: {
...
<div class="content">
    ...
    <div class="panel panel-flat">
        <div class="panel-heading">
            <h5 class="panel-title">News Flash List</h5>
            <div class="heading-elements">
                ...
                @endif
                <a href="{{route('admin.news_flash.addNewsFlash')}}" class="btn btn-labeled btn-labeled-right bg-blue heading-btn ajaxviewmodel">Add News Flash Data<b><i class="icon-menu7"></i></b></a>
            </div>
        </div>
